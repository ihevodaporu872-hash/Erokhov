...
export function loadCalculatorState(data) {
  ...
  if (data.sections && Array.isArray(data.sections)) {
    ...
    sections.forEach(sec => {
      ...
      // Миграция: distributionType (нижний розлив по умолчанию)
      if (sec.distributionType === undefined) {
        sec.distributionType = 'bottom';
      }
...
// Установка схемы розлива ГВС для корпуса
export function setDistributionType(si, type) {
  if (!sections[si]) return;
  sections[si].distributionType = (type === 'top') ? 'top' : 'bottom';
  notifyStateChange();
}
